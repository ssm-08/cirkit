...
def test_first_call_sends_full_prompt(motor):
    inputs = {"context": [_make_signal("The PR diff")]}
...
def test_second_call_sends_delta_when_feedback_changes(motor):
    inputs_1 = {"context": [_make_signal("The PR diff")]}
    state = {"session_id": "test-uuid"}
    with patch("cirkit.llm.call_claude", return_value=_llm_result()):
        motor._call_llm(inputs_1, state)

    inputs_2 = {
        "context": [_make_signal("The PR diff")],
        "feedback": [_make_signal("Please improve section 2")],
    }
    with patch("cirkit.llm.call_claude", return_value=_llm_result()) as mock:
...
def test_second_call_full_prompt_when_all_sections_match(motor):
    """If nothing changes (defensive — cache should block), delta returns None → full prompt sent."""
    inputs = {"context": [_make_signal("Same content")]}
    state = {"session_id": "test-uuid"}
    with patch("cirkit.llm.call_claude", return_value=_llm_result()):
        motor._call_llm(inputs, state)

    # Artificially clear cache so _call_llm runs again with same inputs
...
def test_context_change_included_in_delta(motor):
    inputs_1 = {"context": [_make_signal("Old context")]}
    state = {"session_id": "test-uuid"}
    with patch("cirkit.llm.call_claude", return_value=_llm_result()):
        motor._call_llm(inputs_1, state)

    inputs_2 = {"context": [_make_signal("New context")]}
    with patch("cirkit.llm.call_claude", return_value=_llm_result()) as mock:
        motor._call_llm(inputs_2, state)
