...
- Blocked AND-Gate emits `contradiction=1.0, confidence=0.0` with **real merged content** — NOT `Signal.ZERO` and NOT a `"[BLOCKED]"` placeholder. contradiction triggers R2 cache bypass upstream; merged content gives motors useful feedback to refine against.
...
- **Convergence epsilon by circuit type**: `epsilon: 0.05` for linear pipelines; `epsilon: 0.15` for any circuit with feedback loops. LLM non-determinism causes content drift (0.1/node) that accumulates — epsilon must be above this floor. No need to go below 0.05 for any circuit.
- **`accumulate: false` is redundant**: Battery's `accumulate` defaults to `False` — no need to set it explicitly in JSON.
- **Wire role semantics**: `context` = default for all upstream/sequential wires (battery→motor, motor→gate, gate→sink). `peer` = Motor-to-Motor sibling awareness only (reviewer reading writer's draft). `feedback` = back-edge structural requirement (downstream→upstream). Bidirectional peer creates cycle — use `peer` one direction, `feedback` the other. Only Motor uses role framing — AND-gate, Resistor, Sink ignore role entirely.
...
- **`fake_llm` signature in tests**: `def fake_llm(prompt, *, session_id=None, resume=False, model=None, timeout=60)` — Motor passes all four kwargs; missing `resume` raises `TypeError`.
- **`--session-id` vs `--resume`**: `--session-id <uuid>` CREATES a new session (fails if UUID already exists). `--resume <uuid>` CONTINUES an existing session. Motor uses `--session-id` on iter 1, `--resume` on iter 2+. Getting this wrong causes "Session ID already in use" errors on every circuit with feedback loops.
- **Session-based delta prompting**: each Motor gets a unique UUID `session_id` per `circuit.run()` call (injected by engine bootstrap). On iter 1, Motor sends full prompt via `--session-id`. On iter 2+, sends delta prompt (only changed sections) via `--resume`. Fail-soft: on delta send failure, logs original error and retries with full prompt.
