...
def aggregate_delta(prev_outputs: dict, curr_outputs: dict) -> float:
    ...
    Note: constant-output nodes (e.g. Sink always Signal.ZERO) contribute 0 delta each
    iteration, diluting the aggregate. Effective epsilon sensitivity scales with the
    fraction of actively-changing nodes in the circuit — tune epsilon accordingly.
    """
    if not prev_outputs:
        return 0.0
    if prev_outputs.keys() != curr_outputs.keys():
        raise RuntimeError(
